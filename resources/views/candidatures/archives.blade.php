            ...
            <div class="p-6 text-gray-900">
                @forelse($archives as $a)
                    <div class="border-b border-gray-200 py-4 flex justify-between items-start gap-4">
                        ...
                        <form method="POST" action="{{ route('candidatures.restore', $a->id) }}">
                            @csrf @method('PUT')
                            <button type="submit" class="px-4 py-2 bg-green-600 text-white text-sm rounded hover:bg-green-700">Restaurer</button>
                        </form>
                    </div>
                @empty
